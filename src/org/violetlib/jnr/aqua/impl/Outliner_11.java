  ...
  extends Outliner_10_10
{
    ...
    protected @Nullable Shape getButtonOutline(@NotNull Rectangle2D bounds, @NotNull ButtonLayoutConfiguration g)
    {
        ...
        if (bw == BUTTON_RADIO
          ...
          || bw == BUTTON_ROUND_TEXTURED_TOOLBAR) {
            ...
            y += 1;
            double s = width - 2;
            return new Ellipse2D.Double(x, y, s, s);
        }
        ...
        if (bw == BUTTON_PUSH) {
            ...
            return getRoundedRectangle(bounds, 8, 0.75f);
        ...
        switch (bw)
        {
            ...
            case BUTTON_SEGMENTED_SLIDER_TOOLBAR:
                ...
                if (bw == BUTTON_TAB || bw.isSlider()) {
                    ...
                    if (sz == Size.REGULAR) {
                        if (pos == Position.FIRST || pos == Position.ONLY || pos == Position.MIDDLE) {
                        ...
                        if (pos == Position.ONLY || pos == Position.MIDDLE) {
                            widthDelta = -0.5;
                        } else if (pos == Position.LAST) {
                            widthDelta = 0.5;
                        }
    ...
    protected @Nullable Shape getPopUpButtonOutline(@NotNull Rectangle2D bounds, @NotNull PopupButtonLayoutConfiguration g)
    {
        ...
        if (bw.isTextured()) {
            float corner = size2D(sz, 12, 10, 8, 8);
            return getRoundedRectangle(bounds, corner, 0.75f);
        }
    ...
    protected @Nullable Shape getTextFieldOutline(@NotNull Rectangle2D bounds, @NotNull TextFieldLayoutConfiguration g)
    {
        ...
        Size sz = g.getSize();
        boolean isToolbar = w.isToolbar();
        int version = AquaNativeRendering.getSystemRenderingVersion();

        if (w.isSearch()) {
            double corner = size2D(sz, version < 150000 ? 12 : 8, 8, 6, 4);
            double xAdjust = version >= 150000 ? size2D(sz, 0.5, 0.5, 0.5, 0.5) : 0;
            double widthAdjust = version >= 150000 ? size2D(sz, -1, -1, -1, -1) : 0;
            double yAdjust = size2D(sz, 0.5, 1, 1, 1);
            double heightAdjust = size2D(sz, -1, -2, -2, -2);
            if (isToolbar) {
                xAdjust += size2D(sz, 1, 1, 1, 1);
                widthAdjust += size2D(sz, -2, -2, -2, -2);
                yAdjust -= size2D(sz, 0.5, 1, 1, 1);
                heightAdjust += size2D(sz, 0, 1, 1, 1);
                if (version < 150000) {
                    widthAdjust += 2;
                    xAdjust -= 1;
                }
            ...
            if (version < 150000) {
                if (!isToolbar) {
                    yAdjust -= size2D(sz, 0.5, 0.5, 0.5, 1);
                    heightAdjust += size2D(sz, 0, 0, 0, 1);
                }
                heightAdjust += 1;
            }
            return new RoundRectangle2D.Double(x + xAdjust, y + yAdjust, width + widthAdjust, height + heightAdjust,
              corner, corner);
        } else if (w.isRound()) {
            double corner = size2D(sz, version < 150000 ? 12 : 8, 8, 6, 4);
            double xAdjust = version >= 150000 ? size2D(sz, 1.5, 1.5, 1.5, 1.5) : 1;
            double widthAdjust = version >= 150000 ? size2D(sz, -3, -3, -3, -3) : -2;
            double yAdjust = size2D(sz, 1.5, 1.5, 2.5, 3);
            double heightAdjust = size2D(sz, -3, -3, -4, -4.5);
            if (isToolbar) {
                xAdjust -= size2D(sz, 1, 1, 1, 1);
                widthAdjust += size2D(sz, 2, 2, 2, 2);
                yAdjust -= size2D(sz, 1, 1, 1.5, 2.5);
                heightAdjust += size2D(sz, 2, 2, 2, 3.5);
            }
            if (version < 150000) {
                yAdjust -= 0.5;
                heightAdjust += 1;
